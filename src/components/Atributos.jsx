...
const Atributos = () => {
    ...
    return (
        <div className='relative overflow-x-clip'>
            <img className='max-w-[162px] w-full min-h-[273px] absolute top-[-60%] right-0' src={bottom} alt="bottom" />
            <div className="max-w-[1140px] mx-auto px-3">
                <h2 className='text-[#00141B] pb-[20px] sm:pb-[60px] font-Anton font-normal md:text-[48px] text-[28px] sm:text-[35px] leading-[35px] md:leading-[57px] text-center'>Nuestros Atributos de Marca</h2>
                <div className="flex flex-wrap items-center justify-center flex-row -mx-3 relative z-[2] min-[500px]:justify-between">
                    ...
                    {item.map((p) => <AtributesStructur key={p.id} img={p.Images} naming={p.feature} functioning={p.work} animate={p.animate} />)}
                </div>
            </div>
        </div>
    )
